...
NEURON {
	POINT_PROCESS NmdaCaSyn
	USEION ca READ cai,cao WRITE ica
        RANGE ica
	RANGE B, C, R, R0, R1, g, gmax, lastrelease, onset, releaseTime
...
PARAMETER {
	...
	onset = 0 			: onset time for presynaptic release
	releaseTime = 1 		: duration for release
...
PROCEDURE release() { LOCAL q
	...
	if (q > Deadtime) {
		if (t > onset && t < onset+releaseTime) {		: spike occured?
